...
@mcp.resource("repository://{repo_url}/info")
def get_repository_info(repo_url: str) -> Dict[str, Any]:
    """Basic repository metadata without cloning"""
    try:
        ...
        return {"error": f"Failed to parse repository info: {str(e)}"}

@mcp.resource("repository://{repo_url}/tree?path={path}")
def get_repository_tree(repo_url: str, path: str = "") -> Dict[str, Any]:
    """Get repository file tree structure at a specific path"""
    try:
        cache_key = f"{repo_url}:{path}"
        if cache_key in repo_cache:
            return repo_cache[cache_key]

        entries = []
        if path in repo_cache.get(repo_url, {}):
            entries = repo_cache[repo_url][path]
...
@mcp.resource("repository://{repo_url}/file/{file_path}")
def get_repository_file(repo_url: str, file_path: str) -> Dict[str, Any]:
    """Get metadata about a specific file without content"""
    try:
...
@mcp.resource("repository://{repo_url}/branches")
def get_repository_branches(repo_url: str) -> Dict[str, Any]:
    """Get list of repository branches"""
    try:
        cache_key = f"{repo_url}:branches"
        if cache_key in repo_cache:
            return repo_cache[cache_key]

        result = {
            "branches": [],  # List populated by tool functions
            "timestamp": datetime.now().isoformat()
        }
        repo_cache[cache_key] = result
        return result
    except Exception as e:
        return {"error": f"Failed to get branches: {str(e)}"}

@mcp.resource("repository://{repo_url}/stats")
def get_repository_stats(repo_url: str) -> Dict[str, Any]:
    """Get repository statistics and metrics"""
    try:
        cache_key = f"{repo_url}:stats"
        if cache_key in repo_cache:
            return repo_cache[cache_key]

        result = {
            "created_at": None,  # Populated by tool functions
            "last_updated": None,
            "commit_count": 0,
            "contributor_count": 0,
            "size": 0,
